// v7c/src/signals.mjs
// v7 = v6 lean stream (decideDebounced/momentumOf/flipRisk, BYTE-IDENTICAL to v6/v5.4 —
// preserved for dashboard/runner compatibility: pressure bar, tiles, table, flip readout)
// + a NEW selective, EDGE-SEEKING early-call channel (earlyCallOf, replaced below).
// Headline behavior: fire ONE latched directional call per 5m bar, ASAP within the early
// window, ONLY when the Polymarket favorite price sits in the confident-but-underpriced
// band; abstain otherwise. Requires inp.polyMid (favorite up-probability). 5m-only.
...
Object.assign(CFG, {
  ...
  // v7c early-call channel ('conviction'): SELECTIVE. Fires the market-favored side ONLY
  // when the Polymarket favorite price sits in [0.90,0.96] within the 45-90s fire window,
  // sane read + cushion-agree + dwell 3; ABSTAINS otherwise (no call for the bar).
  // Basis: measured 2026-07-07 on the 883-bar pooled 24/7 evidence base: 90.3% @ 7.0% coverage (94.0% excluding the 07-05 cascade day), median fire ~67s in.
  ...
  EARLY_P_LO: 0.90,        // favorite-price band low (below: market genuinely uncertain, no/neg edge)
  EARLY_P_HI: 0.96,        // band high
...
export function earlyCallOf(s, inp) {
  ...
  if (inBand && cushOk) {
    ...
    if (s.earlyRun >= CFG.EARLY_DWELL) {
      ...
      s.earlyCall = { side, tier: 'conviction', price: Math.round(fav * 1000) / 1000, rem, late };
      return s.earlyCall;
    }
  } else {
