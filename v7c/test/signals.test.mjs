// v7c/test/signals.test.mjs — the conviction early-call channel + engine smoke.
...
test('fires ' + 'conviction' + ' after dwell inside the band, latches immutably', () => {
  const s = newSession();
  ...
  assert.ok(r); assert.equal(r.side, 'UP'); assert.equal(r.tier, 'conviction');
