...
**Standing workflow (2026-07-08): after every pull, run `python3 tools/log-clean/clean-logs.py --pull --execute`** — hard-purges incomplete session logs (gap-runs, early-ends, >30-tick late starts; see `tools/log-clean/README.md`) from repo AND the VM source (else the cron resurrects them). Not an engine change — pure
data hygiene. First purge 2026-07-08: 64 files (34 v53, 7 v54, 15 v6, 3 v7s, 2 v8, 1 v51,
2 eth); every filename + reason in `tools/log-clean/purged-manifest.txt`. Surviving logs
meet a uniform completeness floor: no gap-run >10s, end within 10s of settle, start ≤30
ticks late, ≥80% span density — per-script row filters are no longer needed.
