// v5.4 acceptance gate: replay ALL settled session logs through the REAL v5.3 and v5.4
// engines and compare, split into the 20-bar TUNING set vs OUT-OF-SAMPLE bars.
// Usage: node v5.4/analysis/replay-compare.mjs <logs-dir>
// Read-only on both engines; writes nothing.
import { readFileSync, readdirSync } from 'node:fs';
import { join } from 'node:path';
import * as OLD from '../../v5.3/src/signals.mjs';
...
function loadBars(dir) {
  ...
  for (const f of readdirSync(dir).filter(x => /_v5[123]\.json$/.test(x)).sort()) {
    ...
    if (!settle) continue;
    if (d.rows.filter(r => !r.settled).length < 100) continue;   // exclude test stubs (dataset rule)
...
// DOMINANCE GATE (the 52-bar LHF contract): correct retention >=99% pooled, no bar loses
// >10% of its correct ticks, wrong strictly reduced OR missed converted with wrong not
// increased, transitions <= baseline avg+0.5 and <= baseline max.
