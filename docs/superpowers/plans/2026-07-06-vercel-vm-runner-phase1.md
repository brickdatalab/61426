...
- **Resume guard:** record the engine file's git commit hash in run state + log; on resume refuse if the current hash differs — **and treat a `'unknown'` hash (git unavailable) as a differ / hard stop** — unless `--override-engine-change` is passed.
...
- [ ] **Step 2: Run — expect FAIL.**
> **A1 finding (must honor here):** the early-call latch + momentum warmup are sensitive to `now` *spacing*, not just row order — replaying with a uniform `now+=1000` shifts the warmup-crossing tick and diverges ~0.6% of early-call latches near `rem≤210`. Therefore the runner's **state file rows must persist the actual tick `now` (ms)** (a `now_ms` field, internal to state — NOT added to the public log). Resume-by-replay must feed those exact `now_ms` values back through `tick()` so warmup/momentum/latch reconstruct identically. (The public `<slug>_vX.json` log keeps `t` at HH:MM:SS for format parity.)
> **A2 finding (must honor here):** `Scheduler` seeds its gap baseline from `now()` at start, so on process restart it would report a gap from restart-time, not from the last logged tick. `session.mjs` must compute the resume gap against the **last persisted tick's `now_ms`** (not the scheduler baseline) when writing the single marked gap row.

- [ ] **Step 3: Implement `session.mjs`** — deterministic `playUntilRow`/`playToEnd`/`playFromFeeds` share one internal `_tickWith(now, tape, book, remS)` that: builds `inp` (A1), calls `tick()`, builds the row (round/`toFixed` exactly as the dashboard), records `tape_age_ms`/`book_age_ms` (+ `now_ms` in the state row only), appends to the in-bar buffer, and `writeAtomic`s state every tick. Resume path: read state; if `engineGitHash` differs and no override → throw; else replay buffered in-bar rows through a fresh `newSession()` (reconstructing `s`) before continuing. At `rem<=0`: build settle row `{t, settled: close>open?'UP':'DOWN', open, close}`, write the log **idempotently** (skip if the target log already contains this slug's settle), and if `continuousRemaining>0` advance to `nextSlug`, reset engine session, decrement.
- [ ] **Step 4: Run — expect PASS.**
